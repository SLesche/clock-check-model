...
transformed parameters {
  vector<lower=1>[N] k_pred = 1 + exp(X_k * k);
  vector<lower=0, upper=1>[N] k2_pred = inv_logit(X_k * k2);
  ...
  for (n in 1:N) {
    if (event[n] == 1) {
      target += log_mix(mixture_prob[n], 
        weibull_lpdf(times[n] | k2, lambda2_pred[n]),
        weibull_lpdf(times[n] | k_pred[n], lambda_pred[n])
      );
    } else {
      target += log_mix(mixture_prob[n], 
        weibull_lccdf(times[n] | k2, lambda2_pred[n]),
