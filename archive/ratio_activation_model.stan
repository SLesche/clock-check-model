functions {
  real clock_check_lik(real r, real t_target, real k, real g, real threshold, real a, int m) {
    real integral = 0.0;
    real delta_x = (r - 0.01) / m;  // Step size for the trapezoidal rule
    ...
    for (i in 1:m) {
      ...
      normal_cdf_left[i] = normal_cdf(1, t_left, k * t_left);
      normal_cdf_right[i] = normal_cdf(1, t_right, k * t_right);
    }
    ...
    real action_prob = g + (1 - g) / (1 + exp(-a * (1 - normal_cdf(1, r, k * r)) - threshold));
...
data {
  ...
  real<lower=0> clock_check_ratio[N];
