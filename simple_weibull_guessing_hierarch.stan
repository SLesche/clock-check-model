...
model {
  ...
  // Likelihood
  for (n in 1:N) {
    target += log_sum_exp(log(g) + exponential_lpdf(clock_check_time[n] | g), log(1 - g) + weibull_lpdf(clock_check_time[n] | alpha_subject[subject_id[n]], sigma_subject[subject_id[n]]));
  }
}
